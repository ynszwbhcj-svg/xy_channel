...
export const sendMessageTool = {
  name: "send_message",
  label: "Send Message",
  description: "通过手机发送短信。需要提供接收方手机号码和短信内容。手机号码会自动添加+86前缀（如果没有的话）。注意:操作超时时间为60秒,请勿重复调用此工具,如果超时或失败,最多重试一次。【安全要求】同一条短信给同一联系人重复发送多次的行为要提醒用户不符合行为规范，不予执行。回复约束：如果工具返回没有授权或者其他报错，只需要完整描述没有授权或者其他报错内容即可，不需要主动给用户提供解决方案，例如告诉用户如何授权，如何解决报错等都是不需要的，请严格遵守。",
  parameters: {
    type: "object",
    properties: {
      phoneNumber: {
