...
import { updateTaskIdOnly } from "../task-manager.js";
...
// Dynamic lookup: use latest taskId from task-manager (handles steer/interrupt)
    ...
    const command = {
      ...
      payload: {
        query: params.query,
        sessionId: sessionId,
        interactionId: taskId, // taskId corresponds to interactionId; use dynamic lookup for steer safety
      },
    };
      ...
      const handler = (wrapper: any) => {
        ...
        if (
          ...
          if (event.payload?.isFinal === true) {
            ...
            if (eventTaskId) {
              updateTaskIdOnly(sessionId, eventTaskId, eventMessageId ?? messageId);
            }
